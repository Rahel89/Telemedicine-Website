...
export const viewDoctors = () => {
  return async (dispatch) => {
    ...
    try {
      const response = await axios.get('https://healtlink.onrender.com/api/admin/doctors');
...
export const verifyDoctor = (doctorId) => {
  return async (dispatch) => {
    try {
      await axios.put(`https://healtlink.onrender.com/api/admin/doctors/verify/${doctorId}`);
...
export const unverifyDoctor = (doctorId) => {
  return async (dispatch) => {
    try {
      await axios.put(`https://healtlink.onrender.com/api/admin/doctors/unverify/${doctorId}`);
