...
export const signIn = (email, password) => {
  return async (dispatch) => {
    try {
      const response = await axios.post('https://healt-link-v36m.onrender.com/api/auth/patient/signin', { email, password });
...
export const signUp = (name, email, password) => {
  return async (dispatch) => {
    try {
      const response = await axios.post("https://healt-link-v36m.onrender.com/api/auth/patient/signup", { name, email, password });
...
export const completeDetails = (formData) => {
  return async (dispatch) => {
    try {
      ...
      const response = await axios.put("https://healt-link-v36m.onrender.com/api/auth/patient/details", formData, config);
...
export const updateUserData = (formData) => {
  return async (dispatch) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.put("https://healt-link-v36m.onrender.com/api/auth/updatePatient", formData, {
        headers: {
...
export const logout = (navigate) => { // Accept navigate function as argument
  return async (dispatch) => {
    try {
      await axios.post("https://healt-link-v36m.onrender.com/api/auth/patient/signout");
...
export const requestAppointment = (appointmentData) => {
  return (dispatch) => {
    const token = localStorage.getItem('token');
    axios.post('https://healt-link-v36m.onrender.com/api/auth/appointment/request', appointmentData, {
      headers: {
        'x-auth-token': token
      }
    })
